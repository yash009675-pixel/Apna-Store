-- Use built-in UUID generation for order numbers.
-- gen_random_bytes() was not available under the function's public search_path,
-- causing authenticated checkout RPC calls to fail before the order was created.

CREATE OR REPLACE FUNCTION public.create_order_secure(p_items jsonb, p_shipping jsonb, p_payment_method text DEFAULT 'cod'::text)
...
begin
  ...
  if trim(p_shipping->>'phone') !~ '^[0-9]{10}$' then raise exception 'Invalid phone number'; end if;
  if trim(p_shipping->>'pincode') !~ '^[0-9]{6}$' then raise exception 'Invalid pincode'; end if;
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    exception when invalid_text_representation then
      raise exception 'Invalid product or variant id';
    end;
    v_qty := greatest(coalesce((v_item->>'quantity')::integer,0),0);
    if v_product_id is null or v_variant_id is null or v_qty < 1 then raise exception 'Invalid cart item'; end if;
  ...
  v_order_number := 'APNA-' || upper(substr(replace(gen_random_uuid()::text,'-',''),1,8));
