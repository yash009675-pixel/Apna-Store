...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const [products,categories,brands,variants]=await Promise.all([
   get("/rest/v1/products?select=id,name,slug,description,price,status,category_id,brand_id&status=eq.active&order=name&limit=60",key),
   ...
   get("/rest/v1/product_variants?select=product_id,size,color,stock&order=product_id&limit=300",key)
  ]);
