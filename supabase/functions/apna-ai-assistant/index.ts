...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const callOpenAI=async(model:string)=>fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${openai}`,"Content-Type":"application/json"},body:JSON.stringify(requestBody(model))});
  let ai=await callOpenAI("gpt-5.6-luna");
  if(!ai.ok && (ai.status===400||ai.status===404||ai.status===429)){
    const first=(await ai.text()).slice(0,500);
    console.error("OpenAI Luna error",ai.status,first);
