  ...
  if(token)try{
   ...
   if(ar.ok){const u=await ar.json(), ps=await get(`/rest/v1/profiles?select=id,full_name,role&id=eq.${encodeURIComponent(u.id)}&limit=1`,key,token), p=ps[0]||{};
    ...
    if(user.role==="customer"&&/order|delivery|return|refund|cancel|payment/i.test(message)){
      try{const os=await get(`/rest/v1/orders?select=id,created_at,status,delivery_status,total,payment_status&user_id=eq.${encodeURIComponent(u.id)}&order=created_at.desc&limit=5`,key,token);user.recent_orders=os.map((o:any)=>({id:o.id,created_at:o.created_at,status:o.status,delivery_status:o.delivery_status,total:o.total,payment_status:o.payment_status}))}catch(e){console.error("AI orders load failed",e)}
    }
    if(user.role==="seller")try{user.seller_products=await get(`/rest/v1/products?select=id,name,description,price,compare_at_price,status,category_id,brand_id,slug&seller_id=eq.${encodeURIComponent(u.id)}&order=name&limit=30`,key,token)}catch(e){console.error("AI seller products load failed",e)}
