...
Deno.serve(async(req:Request)=>{
 ...
 try{
  const b=await req.json(), message=clean(b?.message), page=clean(b?.page,80), mode=clean(b?.mode,60), imageData=typeof b?.image_data==="string"&&b.image_data.startsWith("data:image/")?b.image_data:""; if(!message)return json({error:"Message is required."},400);
