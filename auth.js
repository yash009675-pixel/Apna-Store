...
form.addEventListener("submit",async e=>{e.preventDefault();submit.disabled=true;message.textContent="Please wait…";try{if(signUpMode){const result=await apnaSupabase.auth.signUp({email:email.value.trim(),password:password.value,options:{data:{full_name:fullName.value.trim()},emailRedirectTo:new URL("auth.html",location.href).href}});if(result.error)throw result.error;otpRow.hidden=false;otp.required=true;verify.hidden=false;resendVerify.hidden=false;submit.hidden=true;message.textContent="Verification email sent. Check Inbox, Spam, Promotions and Updates.";return;}const result=await apnaSupabase.auth.signInWithPassword({email:email.value.trim(),password:password.value});if(result.error)throw result.error;await finishAuthenticatedUser(result.data.user);}catch(err){message.textContent=err.message||"Something went wrong. Please try again.";}finally{submit.disabled=false;}});

resendVerify?.addEventListener("click",async()=>{if(!email.value.trim())return;resendVerify.disabled=true;message.textContent="Sending a new verification email…";try{const {error}=await apnaSupabase.auth.resend({type:"signup",email:email.value.trim(),options:{emailRedirectTo:new URL("auth.html",location.href).href}});if(error)throw error;message.textContent="New verification email sent. Check Inbox, Spam, Promotions and Updates.";}catch(err){message.textContent=err.message||"Could not resend the verification email.";}finally{resendVerify.disabled=false;}});verify?.addEventListener("click",async()=>{const token=otp.value.trim();if(!/^\d{6}$/.test(token)){message.textContent="Enter the 6-digit verification code sent to your email.";otp.focus();return}verify.disabled=true;message.textContent="Verifying code…";try{const {data,error}=await apnaSupabase.auth.verifyOtp({email:email.value.trim(),token,type:"email"});if(error)throw error;if(!data.user)throw new Error("Verification failed. Please request a new code.");await finishAuthenticatedUser(data.user);}catch(err){message.textContent=err.message||"Invalid or expired verification code.";}finally{verify.disabled=false;}});
