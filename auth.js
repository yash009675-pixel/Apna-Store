...
form.addEventListener("submit",async(e)=>{e.preventDefault();submit.disabled=true;message.textContent="Please wait…";try{let result;if(signUpMode){result=await apnaSupabase.auth.signUp({email:email.value.trim(),password:password.value,options:{data:{full_name:fullName.value.trim()}}});}else{result=await apnaSupabase.auth.signInWithPassword({email:email.value.trim(),password:password.value});}if(result.error)throw result.error;if(signUpMode&&!result.data.session){message.textContent="Account created. Check your email to confirm, then sign in.";setMode();}else{try{const {data:p}=await apnaSupabase.from("profiles").select("role").eq("id",result.data.user.id).maybeSingle();await claimReferral(result.data.user.id);await migrateGuestWishlist(result.data.user.id);}catch(migrateError){console.error("Guest wishlist migration failed:",migrateError);message.textContent="Signed in, but your saved wishlist could not be synced yet. Your guest wishlist is still on this device.";}location.href=p?.role==="admin"?"admin.html":getPostAuthDestination();}}catch(err){message.textContent=err.message||"Something went wrong. Please try again.";}finally{submit.disabled=false;}});
