...
if(existing?.length){
  info.innerHTML="Order <b>"+esc(o.order_number)+"</b> has been delivered.<div style="margin-top:12px"><b>After-sales requests</b>"+existing.map(r=>"<div class="summary-line"><span>"+esc(r.request_type)+" · "+esc(r.status)+(r.return_shipment_id?" · Reverse shipment created":"")+"</span><small>"+esc(new Date(r.requested_at).toLocaleDateString("en-IN"))+"</small></div>").join("")+"</div>";
...
form.addEventListener("submit",async e=>{e.preventDefault();msg.textContent="Submitting request…";
