...
function renderOrders(orders){
 list.innerHTML=orders.length?orders.map(o=>{
  ...
  return '<article class="checkout-card"><p class="eyebrow">ORDER</p><h2>'+esc(o.order_number)+'</h2><div class="summary-line"><span>Order status</span><b>'+esc(statusLabel(o.status))+'</b></div><div class="summary-line"><span>Delivery</span><b>'+esc(statusLabel(o.delivery_status))+'</b></div><div class="summary-line"><span>Payment</span><b>'+esc(o.payment_method==="cod"?"Cash on Delivery":(o.payment_method||"Online"))+' · '+esc(statusLabel(o.payment_status))+'</b></div>'+items.map(x=>'<div class="summary-line"><span>'+esc(x.product_name)+' × '+Number(x.quantity||1)+'</span><b>'+money(Number(x.unit_price||0)*Number(x.quantity||1))+'</b></div>').join("")+'<div class="summary-line"><span>Subtotal</span><b>'+money(o.subtotal)+'</b></div>'+(Number(o.discount_amount)>0?'<div class="summary-line"><span>Coupon'+(o.coupon_code?" ("+esc(o.coupon_code)+")":"")+'</span><b>−'+money(o.discount_amount)+'</b></div>':"")+'<div class="summary-line"><span>Delivery</span><b>'+(Number(o.delivery_fee)?money(o.delivery_fee):"FREE")+'</b></div><div class="summary-total"><span>Total</span><b>'+money(o.total)+'</b></div><p class="checkout-note">Placed '+esc(new Date(date).toLocaleString("en-IN"))+'</p><a class="primary-btn" href="order-detail.html?id='+encodeURIComponent(o.id)+'">View order details →</a></article>';
