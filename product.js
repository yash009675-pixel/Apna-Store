...
function renderProduct(product,variants=[],images=[]){
 ...
 if(!available.length){el.innerHTML='<div class="product-details"><div class="product-detail-top"><p class="eyebrow">PRODUCT</p><button id="wishlistBtn" class="wishlist-detail">♡ Wishlist</button></div><h1>'+esc(product.name)+'</h1><p class="detail-desc">'+esc(product.description||"This product is currently out of stock.")+'</p><button id="backInStockBtn" class="primary-btn">Notify Me When Available</button></div>';document.getElementById("wishlistBtn").onclick=()=>toggleWishlist(product);document.getElementById("backInStockBtn").onclick=()=>notifyBackInStock(product,null);return;}
