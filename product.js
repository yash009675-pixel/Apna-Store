...
function setProductStructuredData(product,images,price){
...
async function renderRelated(product){const box=document.getElementById("relatedProducts");if(!box||!product.category_id)return;const {data}=await apnaSupabase.from("products").select("id,name,price,compare_at_price").eq("status","active").eq("category_id",product.category_id).neq("id",product.id).order("created_at",{ascending:false}).limit(4);if(!data?.length){box.closest("section")?.remove();return}box.innerHTML=data.map(p=>'<a class="product-card" href="product.html?id='+encodeURIComponent(p.id)+'"><div class="product-image"><span>'+esc(p.name)+'</span></div><div class="product-info"><h3>'+esc(p.name)+'</h3><p class="price">₹'+money(p.price)+'</p></div></a>').join("")}
function renderProduct(product,variants=[],images=[]){
 ...
 if(!available.length){el.innerHTML='<div class="product-details"><div class="product-detail-top"><p class="eyebrow">PRODUCT</p><button id="wishlistBtn" class="wishlist-detail">♡ Wishlist</button><button id="followProductBtn" class="text-btn">＋ Follow Product</button><button id="priceDropBtn" class="text-btn">🔔 Price Drop Alert</button></div><h1>'+esc(product.name)+'</h1><p class="detail-desc">'+esc(product.description||"This product is currently out of stock.")+'</p><button id="backInStockBtn" class="primary-btn">Notify Me When Available</button></div>';document.getElementById("wishlistBtn").onclick=()=>toggleWishlist(product);document.getElementById("followProductBtn").onclick=()=>toggleProductFollow(product);document.getElementById("priceDropBtn").onclick=()=>enablePriceDropAlert(product);document.getElementById("backInStockBtn").onclick=()=>notifyBackInStock(product,null);return;}
 ...
 findVariant();
 const mrp=Number(product.compare_at_price||0),price=Number(product.price||0),discount=mrp>price?Math.round((1-price/mrp)*100):0;
 let gallery=[];
 el.innerHTML='<div class="product-visual"><div class="product-gallery"><div class="product-thumbs" id="productThumbs">'+gallery.map((u,i)=>'<button class="product-thumb '+(i===0?"selected":"")+'" data-gallery="'+i+'" aria-label="Product image '+(i+1)+'"><img src="'+esc(u)+'" alt="'+esc(product.name)+'"></button>').join("")+'</div><button class="product-image product-large '+(gallery.length?"has-image":"")+'" id="mainProductImage" aria-label="Open product image">'+(gallery.length?"":"<span>APNA<br>STORE</span>")+'</button><p class="gallery-note">Tap the main image to zoom</p></div></div><div class="product-details"><div class="product-detail-top"><p class="eyebrow">'+esc(product.category||"PRODUCT").toUpperCase()+'</p><button id="wishlistBtn" class="wishlist-detail">♡ Wishlist</button></div><h1>'+esc(product.name)+'</h1>'+(product.brand?'<p class="product-note">Brand: '+esc(product.brand)+'</p>':"")+'<div class="price-block"><strong>₹'+money(price)+'</strong>'+(mrp>price?'<del>₹'+money(mrp)+'</del><span class="discount-badge">'+discount+'% OFF</span>':"")+'</div><p class="detail-desc">'+esc(product.description||"A carefully selected everyday product from Apna Store.")+'</p><div class="detail-meta"><span>✓ Secure checkout</span><span>✓ Easy returns</span><span>✓ India delivery</span></div><div class="option"><b>Size</b><div class="option-list">'+(allSizes.length?allSizes.map(x=>'<button '+(!available.some(v=>(v.size||"")===x)?"disabled ":"")+'data-size="'+esc(x)+'">'+esc(x)+'</button>').join(""):"<span class=\"product-note\">One size</span>")+'</div></div><div class="option"><b>Color: <span id="colorName">'+esc(color||"Standard")+'</span></b><div class="option-list">'+(allColors.length?allColors.map(x=>'<button class="swatch" '+(!available.some(v=>(v.color||"")===x)?"disabled ":"")+'data-color="'+esc(x)+'">'+esc(x)+'</button>').join(""):"<span class=\"product-note\">Standard</span>")+'</div></div><div class="buy-row"><div class="qty"><button id="minus">−</button><span id="qty">1</span><button id="plus">+</button></div><button class="primary-btn" id="add">Add to bag <span>→</span></button></div><p id="stockInfo" class="product-note"></p><div class="delivery-check"><b>Check delivery</b><div><input id="pincode" inputmode="numeric" maxlength="6" placeholder="Enter 6-digit pincode"><button id="checkPincode" class="text-btn">Check</button></div><small id="pincodeMessage">Delivery availability will be confirmed at checkout.</small></div><div class="product-specs"><h3>Product information</h3><div><span>SKU</span><b id="skuInfo">—</b></div><div><span>Category</span><b>'+esc(product.category||"Apna Store")+'</b></div><div><span>Seller</span><b><a id="sellerStoreLink" href="seller-store.html">Visit seller store →</a></b></div></div></div><section class="related-section section"><div class="section-head"><div><p class="eyebrow">YOU MAY ALSO LIKE</p><h2>Related products</h2></div></div><div class="products" id="relatedProducts"><p class="checkout-note">Loading related products…</p></div></section>';
 ...
 function renderGallery(list){gallery=list;activeImage=gallery[0]||null;thumbBox.innerHTML=gallery.map((u,i)=>'<button class="product-thumb '+(i===0?"selected":"")+'" data-gallery="'+i+'" aria-label="Product image '+(i+1)+'"><img src="'+esc(u)+'" alt="'+esc(product.name)+'"></button>').join("");main.classList.toggle("has-image",!!activeImage);main.style.backgroundImage=activeImage?"url('"+activeImage+"')":"";main.innerHTML=activeImage?"":"<span>APNA<br>STORE</span>";thumbBox.querySelectorAll("[data-gallery]").forEach((b,i)=>b.onclick=()=>setImage(i));}
 function setImage(i){activeImage=gallery[i]||null;if(activeImage){main.style.backgroundImage="url('"+activeImage+"')";main.classList.add("has-image");main.innerHTML="";thumbBox.querySelectorAll("[data-gallery]").forEach((b,j)=>b.classList.toggle("selected",j===i));}}
 main.onclick=()=>{if(!activeImage)return;const overlay=document.createElement("div");overlay.className="image-lightbox";overlay.innerHTML='<button aria-label="Close">×</button><img src="'+esc(activeImage)+'" alt="'+esc(product.name)+'">';overlay.onclick=e=>{if(e.target===overlay||e.target.tagName==="BUTTON")overlay.remove()};document.body.appendChild(overlay)};
 ...
 document.getElementById("wishlistBtn").onclick=()=>toggleWishlist(product);
 ...
 setProductStructuredData(product,[],price);refreshOptions();wishlistState().then(v=>{document.getElementById("wishlistBtn").textContent=v?"♥ Saved":"♡ Wishlist"});renderRelated(product);
}
async function loadProduct(){
 ...
 try{
  ...
  const renderedProduct={...data,category:categoryMap.get(data.category_id)||"Apna Store",brand:brandMap.get(data.brand_id)||""};renderProduct(renderedProduct,vr.data||[],images);if(window.apnaTrackProductView)apnaTrackProductView(data.id);if(window.apnaRecommendationFeed)window.apnaRecommendationFeed(data.id);
