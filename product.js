...
async function loadProduct(){
 ...
 try{
  ...
  const [vr,ir]=await Promise.all([
   ...
   apnaSupabase.from("product_images").select("storage_path,alt_text,sort_order,is_primary").eq("product_id",productId).order("is_primary",{ascending:false}).order("sort_order")
  ]);
  if(vr.error||ir.error)throw vr.error||ir.error;
  const images=(ir.data||[]).map(x=>{const u=apnaSupabase.storage.from("product-images").getPublicUrl(x.storage_path).data.publicUrl;return {url:u,alt:x.alt_text||data.name}}).filter(x=>x.url);
